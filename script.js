...
(function () {
  ...
  function apply(theme, userClick) {
  ...
  opts.forEach((b) =>
    b.addEventListener("click", () => apply(b.dataset.themeSet, true)),
  );
...
(function () {
  ...
  function sync() {
    ...
    side.inert = collapsed; // hidden controls can't be tabbed to
  ...
  // FullCalendar doesn't watch its container, so tell it the width changed
  function refreshCalendar() {
    if (typeof calendar !== "undefined" && calendar) calendar.updateSize();
  }

  btn.addEventListener("click", () => {
    if (isCollapsed()) root.removeAttribute("data-sidebar");
    else root.dataset.sidebar = "collapsed";
    try {
      localStorage.setItem("sidebar", isCollapsed() ? "collapsed" : "open");
    } catch (e) {}
    sync();
    setTimeout(refreshCalendar, 340); // fallback: after the slide finishes
  });
